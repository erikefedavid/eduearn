...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background relative overflow-hidden theme-transition">
      ...
      <nav ref={navbarRef} className="fixed top-0 w-full z-50 glass border-b border-primary/10">
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center shadow-lg shadow-primary/20">
              <Zap className="text-white w-6 h-6" />
            </div>
            <span className="text-2xl font-black tracking-tighter text-primary font-heading">EduEarn</span>
          </div>
          <div className="hidden md:flex items-center gap-8 font-bold text-[10px] uppercase tracking-[0.2em] text-muted-foreground transition-colors">
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <Link href="/dashboard">
                <Button className="btn-gradient text-white rounded-xl px-8 font-bold h-12">Dashboard</Button>
              </Link>
            ) : (
              <>
                <Link href="/login">
                  <Button variant="ghost" className="font-bold text-muted-foreground text-xs uppercase tracking-widest">Sign In</Button>
                </Link>
                <Link href="/signup">
                  <Button className="btn-gradient text-white rounded-xl px-8 font-bold h-12 transition-all active:scale-95 shadow-xl shadow-primary/10">Sign Up</Button>
                </Link>
              </>
            )}
          </div>
